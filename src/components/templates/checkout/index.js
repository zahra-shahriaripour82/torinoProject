...
const {data:user}=useGetUserData();
    ...
    const {register,handleSubmit,reset, formState: { errors },}=useForm({resolver: yupResolver(personalDataSchema),defaultValues:{
...
<div className="sm:flex max-w-[1440px] mx-8 md:mx-auto gap-6">
    <form onSubmit={handleSubmit(infoHandler)} className="  border border-border rounded-[10px] text-sm md:text-xl mt-6">
        <div className="p-4">
   ...
   <div className=" md:w-2/6">
    <div className="border border-border  mt-6 px-2 py-4    rounded-xl divide-y-2 divide-dashed ">
        ...
        <button    onClick={checkoutHandler}   className="bg-primary rounded-md py-2 px-4 text-white">ثبت و خرید نهایی</button>
    </div>
    </div>
   </div>
</div>
